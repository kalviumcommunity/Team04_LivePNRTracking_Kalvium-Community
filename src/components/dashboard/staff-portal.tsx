...
export function StaffPortal({ 
  ...
  async function fetchDutyRosterAndAttendance() {
    ...
    setShifts(roster);
    // Restore active check-in state (survives page refresh)
  ...
  async function fetchLuggage() {
    ...
    const list = await getLuggageList(selectedStation);
    setLuggageList(list);
    setLuggageLoading(false);
  }

  // --- Feature 6: Train Passengers List ---
  const [trainsList, setTrainsList] = useState<{ trainNo: string; trainName: string }[]>([]);
  const [selectedTrainNo, setSelectedTrainNo] = useState("");
  const [trainPassengers, setTrainPassengers] = useState<ManifestPassenger[]>([]);
  const [loadingTrainPassengers, setLoadingTrainPassengers] = useState(false);

  async function fetchTrains() {
    const list = await getUniqueTrains();
    setTrainsList(list);
    if (list.length > 0 && !selectedTrainNo) {
      setSelectedTrainNo(list[0].trainNo);
    }
  }

  async function fetchPassengersForTrain(trainNo: string) {
    if (!trainNo) return;
    setLoadingTrainPassengers(true);
    const list = await getTrainPassengers(trainNo);
    setTrainPassengers(list);
    setLoadingTrainPassengers(false);
  }

  const handleUpdateTrainPassengerStatus = async (id: string, newStatus: ManifestPassenger["status"]) => {
    setTrainPassengers((prev) =>
      prev.map((p) => (p.id === id ? { ...p, status: newStatus } : p))
    );
    onUpdatePassengerStatus(id, newStatus);
  };
  ...
  useEffect(() => {
    setTimeout(() => {
      if (activeSubTab === "ops") {
        ...
        void fetchLuggage();
      } else if (activeSubTab === "trainPassengers") {
        void fetchTrains();
      }
    }, 0);
  ...
  // --- Feature 3: Update Incident Status ---
  const handleUpdateIncidentStatus = async (
    incidentId: string,
    currentStatus: string
  ) => {
    const statusCycle: Record<string, "Reported" | "In Progress" | "Resolved"> = {
    ...
    setIncidentLoading(true);
    const res = await updateIncidentStatus(incidentId, nextStatus);
    if (res.success) {
      ...
      alert(res.error || "Failed to update incident status.");
    }
    setIncidentLoading(false);
  };
  ...
  return (
    <div className="space-y-6">
                ...
                {stationPassengers.length > 0 ? (
                  stationPassengers.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors text-sm text-slate-700 dark:text-slate-300">
                      ...
                      <td className="px-6 py-3">
                        <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-semibold ${p.mealStatus === "Delivered"
                ...
                {luggageList.length > 0 ? (
                  luggageList.map((lug) => (
                    <div key={lug.id} className="p-3 border border-slate-105 rounded-lg bg-slate-50/50 dark:bg-slate-900/30 text-xs flex justify-between items-start">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${lug.status === "Delivered"
                          ? "bg-emerald-100 text-emerald-800"
                          : lug.status === "Loaded"
                            ? "bg-blue-100 text-blue-800"
      ...
      {/* --- SUB TAB 6: TRAIN PASSENGERS LIST --- */}
      {activeSubTab === "trainPassengers" && (
        <div className="space-y-6">
          <Card className="border border-[#eaddcd] dark:border-slate-800 bg-white/70 dark:bg-slate-950/40 shadow-sm">
      ...
      {/* Global Seat Re-allocation Modal */}
      {selectedNoShow && (
